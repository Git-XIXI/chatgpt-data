...
import com.chatgpt.data.domain.order.service.IOrderService;
import com.chatgpt.data.trigger.http.dto.SaleProductDTO;
...
public class SaleController {
    ...
    public Response<String> createparorder(@RequestHeader("Authorization") String token, @RequestParam Integer productId) {
        try {
            ...
            assert null != openId;
            log.info("用户商品下单，根据商品ID创建支付订单开始 openId：{} productId：{}", openId, productId);
    ...
    public void payNotify(@RequestBody String requestBody, HttpServletRequest request, HttpServletResponse response) throws IOException {
        try{
            ...
            // 微信传递过来的签名
            String signature = request.getHeader("Wechat-Signature");
            // 证书序列号
            ...
            // 构造 ReauestParam
            com.wechat.pay.java.core.notification.RequestParam requestParam = new com.wechat.pay.java.core.notification.RequestParam.Builder().serialNumber(serialNo)
                    .nonce(nonceStr)
                    .signature(signature)
                    .timestamp(timestamp)
                    .body(requestBody)
            ...
            // 以支付通知回调为例，验签，解密并转换为 Transaction
